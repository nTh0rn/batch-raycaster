...
setlocal enableextensions EnableDelayedExpansion & chcp 65001 & cls
...
:read_map
	...
	:loop
		...
		if "!char!"=="P" (set "pcord=x%x%y%y%" & set /a "px=!x!" & set /a "py=!y!")
		set /a x=%x%+1
	IF NOT "!tmp!" == "!strterm!" GOTO loop
	goto :eof
:print_map
	set screen=
	for /l %%y in (0, 1, 10) do (for /l %%x in (0, 1, 10) do (set screen=!screen!!mapx%%xy%%y!
...
:raycast
	...
	for /l %%y in (1, 1, 46) do (
		...
		if "%1"=="cv" (if "!priority!"=="x" (set /a "nx=!xdir!*!oct_angle!" & set /a "ny=!ydir!*45") else (set /a "ny=!ydir!*!oct_angle!" & set /a "nx=!xdir!*45")
			...
			for /l %%x in (1, 1, 30) do (set /a "low=!toberooted!/!high!" & set /a "high=!low!+!high!" & set /a "high=!high!/2")
	...
	goto :eof
		:v_raycast
			...
			if "!mapx%checkx%y%checky%!"=="·" (set mapx%checkx%y%checky%=#) else if "!mapx%checkx%y%checky%!"=="P" (set filler=0) else if "!mapx%checkx%y%checky%!"=="#" (set filler=0) else (set "walltype=!mapx%checkx%y%checky%!" & call :draw_line & goto :eof)
...
:draw_line
	set /a "tvx=!px!*!scale!+!scale_half!" & set /a "tvy=!py!*!scale!+!scale_half!" & set /a "vx=!vx!-!tvx!" & set /a "vy=!vy!-!tvy!"
